...
const order = ['projects-filters', 'live-preview', 'analytics-consent']
...
test('integrated Studio sections always resolve to the stable accessible vertical order', () => {
  const result = composeWorkspaceSections(
    [section('analytics-consent'), section('projects-filters'), section('live-preview')],
    order,
  )
...
test('duplicate or incomplete section composition is rejected instead of silently overwriting a slot', () => {
  ...
  assert.throws(
    () => composeWorkspaceSections([section('projects-filters'), section('live-preview')], order),
