...
export const projectType = defineType({
  ...
  fields: [
    ...
    defineField({name: 'seoTitle', title: 'Titel i Google', type: 'string', group: 'seo', description: 'En tydlig titel för sökresultatet. Sikta på högst 60 tecken.', validation: (Rule) => Rule.max(60).custom((value, context) => context.parent?.status !== 'published' || value ? true : 'Ett publicerat projekt behöver en titel i Google.')}),
    defineField({name: 'seoDescription', title: 'Beskrivning i Google', type: 'text', group: 'seo', rows: 3, description: 'En mänsklig sammanfattning som får någon att vilja öppna sidan. Sikta på högst 160 tecken.', validation: (Rule) => Rule.max(160).custom((value, context) => context.parent?.status !== 'published' || value ? true : 'Ett publicerat projekt behöver en beskrivning i Google.')}),
  ],
