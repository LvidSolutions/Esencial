...
function transformProjects(result) {
  ...
  for (const source of result) {
    ...
    const images = [source.heroImage, ...visibleGallery].filter((image) => image?.src);
    projects[source.language].push({...source, galleryImages, legacyImages, images: images.length ? images : legacyImages});
  }
