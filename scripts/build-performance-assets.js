...
for (const [src, image] of [...images].sort(([left], [right]) => left.localeCompare(right))) {
  ...
  for (const requestedWidth of widths) {
    ...
    const args = [sourceFile, "-resize", `${requestedWidth}x>`, "-strip"];
    if (drawing) args.push("-define", "webp:lossless=true");
    else args.push("-quality", "90");
    args.push(destination);
    execFileSync("magick", args);
    variants.push({ src: `/assets/images/grid/${filename}`, width, height, bytes: fs.statSync(destination).size });
  }
  entries[src] = {
    src: variants[0].src,
