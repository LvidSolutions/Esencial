const fs = require("fs");
const path = require("path");
const { chromium } = require("playwright");
const {
  AUDIT_DIR,
  BASE_URL,
  LOCAL_ORIGIN,
  REQUIRED_PATHS,
  readJson,
  startStaticServer,
  settlePage,
  browserLaunchOptions
} = require("./recovery-utils");

const SELECTORS = [
  ["body", "body"],
  ["header", ".css_header"],
  ["nav", ".css_nav_container"],
  ["logo", ".css_logo_svg"],
  ["tag filter", ".css_tag_item"],
  ["grid container", ".css_grid_container"],
  ["grid card", ".css_grid_card_container"],
  ["grid photo", ".css_grid_photo"],
  ["grid text", ".css_grid_text_container"],
  ["grid title", ".css_grid_text_name"],
  ["about text", ".css_about_text_container, .css_text_container"],
  ["social icon", ".css_nav_footer_icon_item"],
  ["first paragraph", "p"],
  ["footer", ".css_nav_footer_container"]
];

const PROPS = [
  "font-family",
  "font-size",
  "font-weight",
  "line-height",
  "letter-spacing",
  "color",
  "background-color",
  "margin",
  "padding",
  "display",
  "position",
  "width",
  "height",
  "object-fit",
  "background-size",
  "background-position",
  "opacity",
  "transform",
  "transition"
];

async function inspect(page, url) {
  await page.goto(url, { waitUntil: "domcontentloaded", timeout: 45000 });
  await settlePage(page);
  return page.evaluate(({ selectors, props }) => {
    const out = {};
    for (const [label, selector] of selectors) {
      const el = document.querySelector(selector);
      if (!el) {
        out[label] = null;
        continue;
      }
      const cs = window.getComputedStyle(el);
      out[label] = {};
      for (const prop of props) out[label][prop] = cs.getPropertyValue(prop);
    }
    return out;
  }, { selectors: SELECTORS, props: PROPS });
}

async function main() {
  const state = readJson(path.join(AUDIT_DIR, "crawl-state.json"), { pages: [] });
  const paths = [...new Set([...REQUIRED_PATHS, ...state.pages.map(page => page.path)])];
  const server = await startStaticServer(3000);
  const browser = await chromium.launch(browserLaunchOptions());
  const rows = [];

  try {
    const context = await browser.newContext({ viewport: { width: 1440, height: 1200 }, ignoreHTTPSErrors: true });
    for (const pagePath of paths) {
      const livePage = await context.newPage();
      const localPage = await context.newPage();
      const live = await inspect(livePage, new URL(pagePath, BASE_URL).href);
      const local = await inspect(localPage, new URL(pagePath, LOCAL_ORIGIN).href);
      await livePage.close();
      await localPage.close();

      for (const [label] of SELECTORS) {
        const liveStyles = live[label];
        const localStyles = local[label];
        if (!liveStyles && !localStyles) continue;
        const diffs = [];
        for (const prop of PROPS) {
          if ((liveStyles && liveStyles[prop]) !== (localStyles && localStyles[prop])) {
            diffs.push(`${prop}: live "${liveStyles ? liveStyles[prop] : "missing"}" / local "${localStyles ? localStyles[prop] : "missing"}"`);
          }
        }
        rows.push({ page: pagePath, element: label, differences: diffs });
      }
    }
    await context.close();
  } finally {
    await browser.close();
    server.close();
  }

  const report = [
    "# Computed Style Report",
    "",
    "Compared key element computed styles at 1440x1200.",
    "",
    ...rows.map(row => [
      `## ${row.page} - ${row.element}`,
      "",
      row.differences.length ? row.differences.map(d => `- ${d}`).join("\n") : "- No computed style differences detected for inspected properties.",
      ""
    ].join("\n"))
  ];
  fs.writeFileSync(path.join(AUDIT_DIR, "computed-style-report.md"), report.join("\n"), "utf8");
  fs.writeFileSync(path.join(AUDIT_DIR, "computed-style-report.json"), `${JSON.stringify(rows, null, 2)}\n`, "utf8");
  console.log(`Computed style comparisons written for ${rows.length} element/page pairs.`);
}

main().catch(error => {
  console.error(error);
  process.exit(1);
});
